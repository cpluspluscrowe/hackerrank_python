...
object Result {
  ...
  def getPlayerScoreRank(
      newScore: Int,
      sortedScores: SortedMap[Int, Int],
      accumulations: Array[Int]
  ): (Int, SortedMap[Int, Int], Array[Int]) = {
    val decrementFinalRankBy = getAccumulation(newScore, accumulations)
    val lowerScoreBy = getAccumulation(newScore, accumulations)
    val existingLowerOrEqualScore: Int =
      getPrecedingScore2(newScore, sortedScores.keySet)
    ...
    return (
      newScoreRank + decrementFinalRankBy,
      newTreeMap,
      accumulations :+ newScore
    )
  }
  def climbingLeaderboard(
    ...
    for (playerScoreIndex <- 0 until player.size) {
      ...
      val currentPlayerRankData =
        getPlayerScoreRank(playerScore, sortedScores, accumulations)
